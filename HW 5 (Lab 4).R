...
#Fall 2023
#Lab 4
#Md Muhibul Islam; Joel Rojas

install.packages("stargazer")
#For this lab, we do some regression models to explain wages. We want to understand how important is a college degree 
#(and what major). These sort of regressions are commonly called "Mincer" models after Jacob Mincer who developed them.
...
load("C:/Fall 2023/Econometrics/acs2017_ny_data.RData")
...
#Otherwise the regression would be trying to explain why so many people make zero wages despite so many qualifications. 
#(Think through: if you include retirees in the data and estimate a linear model of age affecting wage, 
#what would you expect for the sign of the coefficient on age?) Of course when you look at college major, 
#you're looking at only people with at least a college degree but you can code that for yourself.

summary(dat_use)
#First, *before running a regression,* consider what variables should be in your model. What are some of the important 
#factors that influence a person's wage? Is there a plausible causal link from X variables to Y and not the reverse?
...
# ^^ that looks like crap since Wages are so skew!  So try to find some sensible ylim = c(0, ??)
plot(INCWAGE ~ jitter(AGE, factor = 2), pch = 16, col = rgb(0.5, 0.5, 0.5, alpha = 0.2), ylim = c(0,150000), data = dat_graph)
# discuss what you see in this plot
